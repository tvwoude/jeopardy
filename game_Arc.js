...
const jeopardyCategories = [
  {
    ...
    questions: [
      { q: "This large bird is the national symbol of the United States.", a: "What is the bald eagle?", value: 100 },
      { q: "This flightless bird is native to Australia.", a: "What is the emu?", value: 200 },
      { q: "This bird can mimic human speech.", a: "What is a parrot?", value: 300 },
      { q: "This nocturnal bird is known for its hooting call.", a: "What is an owl?", value: 400 },
      { q: "This fastest bird dives at over 200 mph.", a: "What is the peregrine falcon?", value: 500 }
    ]
  },
  {
    name: "Muscle Matters",
    questions: [
      { q: "This is the strongest muscle in the body relative to its size.", a: "What is the masseter (jaw muscle)?", value: 100 },
      { q: "This muscle group straightens the knee.", a: "What are the quadriceps?", value: 200 },
      { q: "The Achilles tendon attaches this muscle to the heel.", a: "What is the calf?", value: 300 },
      { q: "This organ is technically the hardest working muscle.", a: "What is the heart?", value: 400 },
      { q: "The scientific term for the abs.", a: "What is the rectus abdominis?", value: 500 }
    ]
  },
  {
    name: "Mind Games",
    questions: [
      { q: "This Austrian is considered the father of psychoanalysis.", a: "Who is Sigmund Freud?", value: 100 },
      { q: "This part of the brain is responsible for memory.", a: "What is the hippocampus?", value: 200 },
      { q: "BF Skinner is best known for this type of conditioning.", a: "What is operant conditioning?", value: 300 },
      { q: "This psychological effect makes people conform to group opinion.", a: "What is the Asch effect?", value: 400 },
      { q: "Maslow is known for this pyramid-shaped theory.", a: "What is the hierarchy of needs?", value: 500 }
    ]
  },
  {
    name: "Slice of Life",
    questions: [
      { q: "This Italian city is credited with inventing pizza.", a: "What is Naples?", value: 100 },
      { q: "This is the most popular pizza topping in the US.", a: "What is pepperoni?", value: 200 },
      { q: "This style of pizza is known for its deep-dish crust.", a: "What is Chicago-style?", value: 300 },
      { q: "This Italian cheese is traditionally used on pizza.", a: "What is mozzarella?", value: 400 },
      { q: "This chain introduced the ‘30 minutes or less’ guarantee.", a: "What is Domino’s?", value: 500 }
    ]
  },
  {
    name: "Twang Time",
    questions: [
      { q: "This singer is known as the 'King of Country'.", a: "Who is George Strait?", value: 100 },
      { q: "This Nashville street is famous for country music venues.", a: "What is Broadway?", value: 200 },
      { q: "This female singer was known for ‘Jolene’.", a: "Who is Dolly Parton?", value: 300 },
      { q: "This state is home to the Grand Ole Opry.", a: "What is Tennessee?", value: 400 },
      { q: "This country star had a hit with ‘Friends in Low Places’.", a: "Who is Garth Brooks?", value: 500 }
    ]
  }
];
...
function showStartScreen() {
  ...
  if (isHost) {
    ...
    startButton.onmouseover = () => {
      startButton.style.background = "#FFA500";
...
function startGameForHost() {
  ...
  // Show category intro first
  showCategoryIntro(categories);
}
...
function startDoubleJeopardy() {
  ...
  showCategoryIntro(categories);
  sendMessage({ type: "startDouble", categories });
}
    ...
    categories.forEach((cat, cIndex) => {
      ...
      cell.addEventListener("click", () => {
        if (isHost && !cell.classList.contains("used")) {
          ...
          const isDD = dailyDoubles.some( dd => dd.catIndex === cIndex && dd.qIndex === i );
          if (isDD) {
  ...
  if (isHost) {
    ...
    if (round === 1) {
      ...
      dblBtn.onclick = () => {
        round = 2;
        categories = doubleJeopardyCategories;
        pickDailyDoubles(2); // do this first
        buildBoard();         // then buildBoard
        renderScores();
        sendMessage({ type: "startDouble", categories: doubleJeopardyCategories });
      };
...
function pickDailyDoubles(count) {
  ...
  dailyDoubles = Array.from(picks).map(str => {
    ...
    return { catIndex: catIdx, qIndex: qIdx };
  });
...
function showDailyDoubleQuestion(q, a, value, selectedPlayerIndex) {
  ...
  answerTextEl.innerText = ""; // Players don't see the answer
  
  // Remove any old Daily Double UI
...
// ===== SHOW DAILY DOUBLE =====
// Replace the showDailyDouble function with this fixed version:
...
function showDailyDouble(q, a, value) {
  ...
  const baseQuestion = {
    ...
    a,
    value,
    category: currentQuestion?.category ?? 0, // fallback to 0
  ...
  if (isHost) {
    ...
    document.getElementById("dd-lock").onclick = () => {
      ...
      // Mark as used
      sendMessage({ type: "markCell", category: baseQuestion.category, index: baseQuestion.index });
...
function markCellUsed(category, index) {
  if (!categories || !boardEl) return; // early exit
